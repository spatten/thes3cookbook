...
class S3Syncer
  ...
  def initialize(directory, bucket_name, params = {})
    ...
    # sync_params are parameters sent to the S3Object.store 
    # in the sync method
    @sync_params = @params.dup
    [:ignore_extensions, :ignore_regexp, 
     :prefix, :compressed_extensions].each do |key|
      @sync_params.delete(key)
    end
  end
  ...
  def sync
    ...
    @files_to_upload.each do |file|
      params = @sync_params.dup
      data = if file_should_be_compressed?(file)        
        params.merge!('Content-Encoding' => 'gzip')
      ...
      S3Object.store(s3_name(file), data, @bucket_name, params)      
    end
  end
  
  def file_should_be_compressed?(file)
    return false unless @params.has_key?(:compressed_extensions)
    @params[:compressed_extensions].include?(File.extname(file))    
  end
  
  def compressed_file(file)
    strio = StringIO.open('', 'w')
    gz = Zlib::GzipWriter.new(strio)
    gz.write(File.safe_read(file))
    gz.close
    strio.string
  end
  ...
  def s3_name(file)
    File.join(
               @params[:prefix], file.sub(/\A#{@directory}/, '')
             ).sub(/\A\//,'')
  end
